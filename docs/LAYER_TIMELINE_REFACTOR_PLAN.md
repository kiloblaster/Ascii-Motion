...
> **Status:** In Progress — Phase 6 substantially complete (MCP server deferred), performance optimized, ready for Phase 7  
...
| §6.5 MCP server v2 protocol | `ascii-motion-mcp/` | ⬜ DEFERRED to after Phase 7 | Independent codebase. Existing tools work on single-layer via client adapter. Full layer-aware MCP tools deferred until all app-side features are complete. |

### Phase 7: Advanced Layer Features (In Progress)

| Task | File(s) | Status | Notes |
|------|---------|--------|-------|
| Layer menu (hamburger) | `src/components/features/timeline/LayerMenu.tsx`, `TimelineToolbar.tsx` | ✅ DONE | DropdownMenu at leftmost toolbar position with divider. Contains: Merge Down, Merge Visible, Flatten Transforms, Create Group, Ungroup. Disabled states based on active layer context. |
| Layer right-click context menu | `src/components/features/timeline/LayerContextMenu.tsx`, `LayerList.tsx`, `LayerListItem.tsx` | ✅ DONE | Custom portal-based menu (same pattern as TimelineContextMenu). Actions: Rename, Duplicate, Delete, Hide/Show, Solo, Lock/Unlock, Merge Down, Merge Visible, Flatten, Create Group, Ungroup. Triggered by `onContextMenu` on LayerListItem rows. |
| Merge Down store action | `src/stores/timelineStore.ts` | ✅ DONE | Composites upper + lower layers frame by frame, creates merged layer with lower's name, removes both originals. |
| Merge Visible store action | `src/stores/timelineStore.ts` | ✅ DONE | Composites all visible layers frame by frame, creates "Merged" layer, removes all visible layers. |
| Flatten Transforms store action | `src/stores/timelineStore.ts` | ✅ DONE | Bakes transforms into content frame data via compositing, resets all property tracks and static properties. |
| Create Group / Ungroup store actions | `src/stores/timelineStore.ts` | ✅ DONE | Groups store childLayerIds + update parentGroupId on layers. Single nesting only. |
| History types for merge/group | `src/types/index.ts` | ✅ DONE | `MergeLayersHistoryAction`, `CreateGroupHistoryAction`, `UngroupLayersHistoryAction` with full layer snapshots for undo. |
| History wrappers | `src/hooks/useTimelineHistory.ts` | ✅ DONE | `mergeDown`, `mergeVisible`, `createGroup`, `ungroupLayers` wrapped with snapshot-based undo/redo. |
| Undo/redo handlers | `src/hooks/useKeyboardShortcuts.ts` | ✅ DONE | `merge_layers`, `create_group`, `ungroup_layers` cases in `processHistoryAction`. Merge undo restores original layers at original indices. |
| Multi-layer selection | — | ⬜ NOT STARTED | `selectedLayerIds` exists in view state but no actions yet. |
